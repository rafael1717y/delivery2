...
original_author="rafael1717y"
original_name="delivery2"
original_urlname="delivery2"
original_description="Awesome delivery2 created by rafael1717y" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/delivery2 "${name}"
...
then
    ...
    echo "then run 'delivery2' or 'python -m delivery2'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'delivery2' or 'python -m delivery2' or 'poetry run delivery2'"
fi
